...
- Use one identical footer across every knowledge landing page and article. Quick links contain `الرئيسية`, `مركز المعرفة`, `قاموس التعريفات`, `المدونة`, and `اتصل بنا`; do not place `من نحن` in this column.
- Place `من نحن` under `الدعم والسياسات` with `اتصل بنا`, `سياسة الخصوصية`, `شروط الاستخدام`, and `خريطة الموقع`.
- The footer `الأقسام` column must contain all nine lowercase knowledge categories. Render them as two visual columns filled 5+4 in this order: text tools, developer, photo editing, calculators, PDF/docs, ZIP, SEO, general, and social media.
